...
async def start(message):
    if message.text == '/go':
        await bot.send_message(message.from_user.id,
                         ...
                         "1-я картинка - это исходное изображение, "
                         "2-я картинка стиль который будет перенесен")
    else:
...
async def handle_docs_photo(message):
    ...
    if flag == 0:
        ...
        await bot.send_message(message.from_user.id, 'Исходное изображение '
                                                     'загружено')
        flag = 1
        # примем первую пикчу
    elif flag == 1:
        await message.photo[-1].download('style.jpg')
        await bot.send_message(message.from_user.id, 'Изображение стиля '
                                                     'загружено')
